...
def movie_details():
    ...
    movie_details = requests.get(
        'http://' + 'backend:5000' + '/api/v1/searchv2/' + str(movieID)).json()
    print(movie_details)
...
def personality_analysis():
    data = {}
    data['traits'] = requests.get(
        'http://backend:5000/api/v1/traits/get-all-traits').json()
    ...
    if movieID != 0 and title != 0:
        ...
        data['film_traits_ranking'] = requests.get(
            film_traits_ranking_url).json()
    ...
    if request.method == 'POST':
        ...
        if trait_code is not None and highest is not None:
            ...
            data['trait_film_ranking'] = requests.get(
                trait_film_ranking_url).json()
            data['trait_film_ranking_data'] = {"trait": data['traits'][int(
                trait_code)], "highest": ("Highest" if int(highest) == 1 else "Lowest")}
        ...
        if trait_code is not None:
            ...
            data['trait_film_ratings'] = draw_ratings_pie_chart(
                ratings_percentages)
            data['trait_film_ratings_data'] = {
                "trait": data['traits'][int(trait_code)]}
        ...
        if trait_code is not None and highest is not None:
            ...
            data['trait_genre_ranking'] = requests.get(
                trait_genre_ranking_url).json()
            data['trait_genre_ranking_data'] = {"trait": data['traits'][int(trait_code)], "highest": (
                "Highest to lowest" if int(highest) == 1 else "Lowest to highest")}
        ...
        if genreID is not None and highest is not None:
            ...
            data['genre_traits_ranking'] = requests.get(
                genre_traits_ranking_url).json()
            data['genre_traits_ranking_data'] = {"genre": data['genres'][int(
                genreID) - 1], "highest": ("Highest to lowest" if int(highest) == 1 else "Lowest to highest")}
...
def viewer_analytics():
    ...
    # scattler plot
    query_str = 'http://' + 'backend:5000' + \
        '/api/v1/viewer-analysis/ratings/' + str(movie_id)
    scatter_plot_data = requests.get(query_str).json()

    # pie chart
    # bad ratings
    query_str = 'http://' + 'backend:5000' + \
        '/api/v1/viewer-analysis/user-group/' + str(movie_id) + '/1'
    bad_ratings = requests.get(query_str).json()

    # good ratings
    query_str = 'http://' + 'backend:5000' + \
        '/api/v1/viewer-analysis/user-group/' + str(movie_id) + '/2'
    good_ratings = requests.get(query_str).json()

    # amazingratings
    query_str = 'http://' + 'backend:5000' + \
        '/api/v1/viewer-analysis/user-group/' + str(movie_id) + '/3'
    amazing_ratings = requests.get(query_str).json()

    pie_chart_data = [bad_ratings["num_users"],
                      good_ratings["num_users"], amazing_ratings["num_users"]]

    print("pie chart data", pie_chart_data)

    num_users_rated = bad_ratings["num_users"] + \
        good_ratings["num_users"] + amazing_ratings["num_users"]
    # summary statistics
    ...
    # Avg User Rating
    query_str = 'http://' + 'backend:5000' + \
        '/api/v1/viewer-analysis/average-rating/' + str(movie_id)
...
def generate_text_stats(genre_data):
    ...
    text = "Viewers in this group like " + fav_genre + " the most, followed by " + fav_genre_2 + \
        " and " + fav_genre_3 + " movies. Additionally, they dislike " + \
        worst_genre + " movies."
    return text
...
def generate_rating_stats(rating_history, movie_rating):
    ...
    if (agg_avg_rating_history < agg_avg_movie_rating):
        ...
        if (diff > 1):
            text = "On average, viewers in this group scored this movie higher by " + \
                str(round(diff, 2)) + " than their usual rating"
        else:
            text = "On average, viewers in this group scored this movie very similar to their usual rating, however slightly higher by " + \
                str(round(diff, 2)) + " than their usual rating"
    elif (agg_avg_rating_history > agg_avg_movie_rating):
        diff = agg_avg_rating_history - agg_avg_movie_rating
        if (diff > 1):
            text = "On average, viewers in this group scored this movie lower by " + \
                str(round(diff, 2)) + " than their usual rating"
        else:
            text = "On average, viewers in this group scored this movie very similar to their usual rating, however slightly lower by " + \
                str(round(diff, 2)) + " than their usual rating"
...
def group_analysis():
    ...
    if (user_group_stats):
        # line graphs
        query_str = 'http://' + 'backend:5000' + '/api/v1/viewer-analysis/rating-history/' + \
            str(movie_id) + '/' + str(user_group_stats)
        print("query str", query_str)
        avg_rating_line_data = requests.get(query_str).json()
        print("avg line data", avg_rating_line_data)
        query_str_2 = 'http://' + 'backend:5000' + '/api/v1/viewer-analysis/movie-rating/' + \
            str(movie_id) + '/' + str(user_group_stats)
        movie_rating_line_data = requests.get(query_str_2).json()
        print("move line data", movie_rating_line_data)
        # get analysis text
        try:
            text_graphs = generate_rating_stats(
                avg_rating_line_data["avg_ratings"], movie_rating_line_data["ratings"])
        except ZeroDivisionError:
            return redirect("/viewer-analytics?movieID=" + str(movie_id), code=302)

        # genre table
        query_str = 'http://' + 'backend:5000' + '/api/v1/viewer-analysis/genre-rating/' + \
            str(movie_id) + '/' + str(user_group_stats)
