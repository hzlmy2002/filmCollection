...
class GetTagsByGenre(Resource):
    def get(self, genre):
        command = ("SELECT LOWER(User_tags.movielens_tag) AS tag_name, COUNT(Genres.genre) AS tag_count "
                   ...
                   "GROUP BY LOWER(User_tags.movielens_tag), Genres.genre "
        ...
        result_dict = SqlExecutor().convert_to_dict(
            result, ["tag_name", "tag_count", "percentage"])
        return result_dict
